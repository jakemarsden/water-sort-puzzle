...
//	public static String[] puzzle = {"    ","AAAB", "ABBB"};
	...
	public static List<String[]> solutionTubes = new ArrayList<String[]>();
	...
	public static void main(String[] args) 
	{
		...
		// Check that the puzzle supplied is valid (must have at least 2 tubes and all tubes must contain the number of items specified
		// by the depth int.
		if (puzzle.length < 2)
		{
			System.out.println("!!!NOT A VALID PUZZLE!!!");
			System.out.println("2 tubes at least need to be supplied, but " + puzzle.length + " have been supplied.");
			System.exit(1);
		}		
		...
		if (invalidTube != null)
		{
			...
			System.out.println("This tube does not (there may be other invalid tubes)." + System.lineSeparator());
	...
	private static String[] getTubesFrom(String[] step)
	{
		...
		for (int pos = 0 ; pos < tubes.length ; pos++)
			tubes[pos] = step[pos];
		...
		for (int fromTube = 0 ; fromTube < currentTubes.length ; fromTube++)
		{
			for (int toTube = 0 ; toTube < currentTubes.length ; toTube++)
			{
				...
				if (moveIfValid(newTubes, fromTube, toTube))
				{
					if (solve(newTubes))
					{
						addSolutionStep(currentTubes, "Move from " + (fromTube + 1) + " to " + (toTube + 1));
						return true;
					}
					// if we this valid move did ultimately solve the puzzle, we continue looking for other valid moves.
				}				
			}
		}
		
		// We have tried every combination of moving liquids from one tube to another, either there are no valid moves from this
	...
	private static void addSolutionStep(String[] currentTubes, String message)
	{
		...
		for (int pos = 0 ; pos < currentTubes.length ; pos++ )
		{
			step[pos] = currentTubes[pos];
		}
	...
	private static boolean moveIfValid(String[] tubes, int from, int to) {
		...
		// a) Not allowed to entire contents of a tube to an empty tube (this could create infinite loops with liquids being bounced backwards and forwards.)
		...
		// " AABA" to "   AB" -> "    B" "AAABA" (2 A's are moved on-top of the A in the other tube - NOTE we must move both A's).
		// "Example invalid moves:
		// " CCCC" "     " (invalid by a).
		// " CCCC" " CAAA" (invalid by b and d - Must attempt to move all 4 C's and these won't fit as there is only 1 available space, we are not allowed to  move just one).
		// " CCCC" " ACCC" (invalid by c) ).
		// "     " "ABCDE" (invalid by e - can't move from an empty tube, even though it would technically fit as 5 + 0 = 5).
		...
		String newFromTube = padLeft(fromTube.trim().substring(colourToMove.length(), fromTube.trim().length()), depth);
	...
	{
		// Each tube must be fully empty or full of the same letter, if all tubes meet this criteria, then the puzzle is solved.
		for (String tube : tubes)
		{
			// Construct a string which represents what a tube full of the first digit found in the tube would look like.
	...
	{
		String tubeWithNoSpace = tube.trim();
		if (tubeWithNoSpace.equals(stringBasedOnFirstLetter(tubeWithNoSpace)))
			return true;
	...
	private static String stringBasedOnFirstLetter(String tube)
	{
		...
		return new String(new char[tube.length()]).replace('\0', tube.substring(0,1).toCharArray()[0]);
	}
	...
	private static String display(String[] tubes, int depth)
	{
		...
		for (int d = 1 ; d <= depth ; d++)
		{
			...
			for (String tube : tubes)
			{
				...
				visualisation.append(tube.substring(d-1, d));
				visualisation.append(side);
				visualisation.append(spacer);
			}
			visualisation.append(spacer);
